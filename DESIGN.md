...
- **Selección en mobile:** bajo 900px el panel de detalle ya no es la columna sticky de al lado (ver Cartera + detalle arriba); vive apilado debajo de las 30 tarjetas de empresa, lo que en la práctica significa varios miles de píxeles de distancia entre tocar una fila y ver su resultado. `selectCompanyRow` llama a `scrollToDetailOnMobile()`, que hace `scrollIntoView({behavior: 'smooth'})` hacia `#company-detail` solo bajo ese breakpoint (respeta `prefers-reduced-motion`). Sin esto, seleccionar una empresa "no hacía nada" visible — la razón explicable sí se renderizaba, solo que muy lejos de donde estaba la atención del usuario.
...
Franja fija (`position: fixed`, `top: 0`) con tres métricas clave (tasa de aprobación, cupo total, pérdida esperada) que aparece solo mientras el Panel de reglas está en pantalla, vía `IntersectionObserver` sobre `#controls` — no compite por espacio el resto del scroll. Existe porque en mobile (una sola columna, seis controles apilados) mover un slider y ver su efecto en Resultados implicaba scrollear de ida y vuelta; esta franja trae el resultado al control en vez de mandar al usuario a buscarlo. Reutiliza `formatPercent`/`formatCurrency` dentro de `renderMetrics()`, así que nunca se desincroniza de las tarjetas de Resultados. `left: 240px` en desktop (no tapa el sidebar), `left: 0` bajo 900px.
